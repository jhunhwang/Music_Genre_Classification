...
def output_music_csv(artist):
  # Append the artists that were entered by the user in the ``artist_name`` list.
  music_info['call_data']['kpop_western'].append(artist)
  ...
  kpop = ['ITZY', 'DREAMCATCHER', 'TWICE']
  western = ['Chainsmokers']
  ...
  song_df = song_df.drop_duplicates(subset=['duration_ms', 'danceability', 'energy', 'loudness', 'speechiness', 'acousticness', 'instrumentalness', 'liveness', 'valence', 'tempo'], keep='first')
  
  return song_df 

match_df = pd.read_csv('./test_storage/test_music_features.csv')
...
def test_run_song(artist, expected):
  OUTPUT = output_music_csv(artist)
  OUTPUT.to_csv('./test_storage/test_music_features(v1).csv', index=False)
  output_df = pd.read_csv('./test_storage/test_music_features(v1).csv')
  assert expected.equals(output_df)